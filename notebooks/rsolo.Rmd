...
```
## Which school had the largest drop from `alg_1` to `alg_2`? For what percentage of schools is it true that `alg_2` is larger than `alg_1`? 
```{r}
districts %>% 
  arrange(desc(alg12_diff)) %>% 
  slice_head()
...
```{r}
districts %>% 
  filter(eng23_diff != 'NA') %>% 
  arrange(desc(eng23_diff)) %>% 
  ggplot(aes(x = eng23_diff)) +
  geom_histogram(color = 'white')
```
...
```{r}
grad_suspended_lm <- lm(grad ~ suspended , data = districts) #lm provides average expected values (but mean is not all the story)
grad_suspended_lm
summary(grad_suspended_lm)
```
```{r}
grad_suspended_predicted_values <- predict(grad_suspended_lm , data = districts)
grad_suspended_predicted_values
```
```{r}
districts %>% 
  mutate(prediction = predict(grad_suspended_lm , newdata = districts)) %>% 
  ggplot(aes(x = grad, y = suspended)) +
  geom_point() +
  geom_line(aes(x = grad, y = prediction))
...
districts %>% 
  ...
  ggplot(aes(x = grad, y = suspended)) +
  geom_point() +
  geom_line(aes(x = grad, y = fit)) +
  geom_ribbon(aes(ymin = lwr, ymax = upr), fill = 'cornflowerblue', alpha = 0.2)
```
```{r}
districts %>%  #reasonable range of values for the variable that 95 percent of cases will be within this band
  cbind(predict(grad_suspended_lm, districts, interval = c('prediction'), level = 0.95)) %>% 
  ggplot(aes(x = grad, y = suspended)) +
  geom_point() +
  geom_line(aes(x = grad, y = fit)) +
  geom_ribbon(aes(ymin = lwr, ymax = upr), fill = 'cornflowerblue', alpha = 0.2)
