...
```{r}
grad_min <- min(districts[,'grad'], na.rm = TRUE)
grad_min
```
```{r}
districts %>%
...
```{r}
dropout_max <- max(districts[, 'dropout'], na.rm = TRUE)
dropout_max
```
...
```
```{r}
ggplot(enrollment_by_region, aes(x= region) +
geom_bar()
```
...
```
## Q17:For many districts, values for `alg_2` are lower than for `alg_1`. Create a histogram showing the distribution of differences (`alg_1` - `alg_2`). Which school had the largest drop from `alg_1` to `alg_2`? For what percentage of schools is it true that `alg_2` is larger than `alg_1`? Is there a similar dropoff for `eng_2` and `eng_3`?
```{r}
